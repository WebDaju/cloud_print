...
const Orders: NextPageWithLayout<OrderProps> = () => {
  const router = useRouter();
  const { id } = router.query;
  const { data } = api.order.getOrderById.useQuery({
    id: Number(id),
  });
  ...
  const handleUpdateProduct = async (e: React.FormEvent) => {
    const data = { id: Number(id), status: status };
        ...
        <Card className="m-auto w-3/4 shadow-md">
          <div className="flex justify-between p-10">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-lg  ">
              <p className="text-lg">Phone: {data?.orders?.phone?.toString()}</p>
          ...
          <div>
            <h1 className="text-black text-2xl text-center p-10">
              Products
              <hr></hr>
            </h1>
